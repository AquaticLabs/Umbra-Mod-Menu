...
namespace UmbraRoR
{
    public class PlayerMod : MonoBehaviour
    {
        ...
        public static void GiveBuff(GUIStyle buttonStyle,string buttonId)
        {
            ...
            for (int i = 0; i < Enum.GetNames(typeof(BuffIndex)).Length; i++)
            {
                string buffName = Enum.GetNames(typeof(BuffIndex))[i];
        ...
        public static void ChangeCharacter(GUIStyle buttonStyle, string buttonId)
        {
            ...
            for (int i = 0; i < Main.bodyPrefabs.Count; i++)
            {
                DrawMenu.DrawButton(buttonPlacement, buttonId, Main.bodyPrefabs[i].name.Replace("Body", ""), buttonStyle);
                buttonPlacement++;
            }
